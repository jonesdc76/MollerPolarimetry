/*
Plots the saturation magnetization data for Fe from numerous papers including

Sanford et al 1941 "A determination of the magnetic saturation induction of iron at room temperature"

Danan 1959 "On the interpretation of the magnetization measurements of pure polycrystalline iron and nickel in the vicinity of saturation"

Arajs et al 1967 "Electrical Resistivity Studies of Chromium-Rich Chromium-Cobalt Alloys

Crangle et al 1971 "The Magnetization of Pure Iron and Nickel"

Behrendt et al 1972 "Saturation Magnetization of Polycrystalline Iron"

Plots are converted where possible to be versus applied field H
*/

#include <iostream>
#include <cstdio>
#include "TGaxis.h"
#include "TAxis.h"
#include "TGraph.h"
#include "TGraphErrors.h"
#include "TCanvas.h"
#include "TStyle.h"
#include "TF1.h"
#include "TArrow.h"
#include "TMultiGraph.h"
#include "TPaveText.h"
#include "TLegend.h"

//Returns theoretical magnetization value for given Hi in KOe, T in Kelvin
//Curves from Pauthenet Mar 1982 "Spin Waves in nickel, iron and yttrium-iron
//garnet" equation 9 and Table 1
//Adjustable parameter is spontaneous magnetization (nominally 222.678 emu) 
double magnetization( double Hi, double T, double spont_m = 222.678,
		      int nTerms = 10000){
  //calculate 307e-6*T^(3/2)*F(3/2, 1.378*Hi/T) from equation (9) at T and Hi
  const double a3_2 = -307e-6, a5_2 = -22.8e-8, b = 1.378e-4;
  double s = 3.0/2.0, term1 = 0, term2 = 0, term3 = 0;
  for(int i=1;i<nTerms;++i){
    term1 += a3_2*pow(T,s)*pow(i,-s)*exp(-i*b*Hi*1000.0/T);
  }
  s = 5.0/2.0;
  for(int i=1;i<nTerms;++i){
    term2 += a5_2*pow(T,s)*pow(i,-s)*exp(-i*b*Hi*1000.0/T);
  }
  //my linear parameterization of Chi term using fit to values in Table 1
  double intercept = 3.644e-6, slope = 5.0434e-10;
  term3 = (intercept + slope * T) * Hi*1000.0;
  double mag = spont_m + term1 + term2 + term3;
  return mag;
}

void comboplot(bool use_Hi = true){
  ////////////////////////////////////////////////////////////////////////
  ///////////////////////// NEW STYLING //////////////////////////////////
  ////////////////////////////////////////////////////////////////////////
  bool new_style = true;
  Int_t icDark  = 11000; //Phthalo blue
  TColor *cDark = new TColor(icDark,  20./255. ,  20./255. , 135./255. );
  Int_t icMed   = 11001; //Auburn red
  TColor *cMed  = new TColor(icMed,  182./255. ,  64./255. ,  64./255. );
  Int_t icLite  = 11002; //Mint green
  TColor *cLite = new TColor(icLite,  51./255. , 205./255. ,  51./255. );
  Int_t icSoft  = 11003; //Soft Soft Blue -- This could ideally be used as a background using a fill style
  TColor *cSoft = new TColor(icSoft, 156./255. , 238./255. , 241./255. );
  Int_t icLGray = 11004;
  TColor *cLGray= new TColor(icLGray,220./255. , 220./255. , 220./255. , "" , 0.5 );
  //Top2Bottom Fit order= (5,       2,      6,       3,     4,       1);
  //Legend Name           {WEISS,   NIST,   DANAN,   ARAJS, CRANGLE, NASA,    kOrange+7, 1};
  Int_t   markstyle[7]  = {39,      21,     8,       49,    33,      47,      26};
  Float_t markersize[7] = {2.0,     1.5,    1.25,    2.,    2.,      1.5,     1.5};
  Int_t   linestyle[7]  = {1,       7,      7,       5,     7,       1,       9};
  Int_t   linewidth[7]  = {2,       2,      2,       2,     2,       3,       2};
  Int_t   color1[9]      = {icLite,  icLite, icMed,   icMed, icDark,  icDark,  11001,     1};
  /* Each graph will require:
     gr->SetMarkerColor(color[n]);
     gr->SetMarkerSize(markersize[n]);
     gr->SetMarkerStyle(markstyle[n]);
     gr->SetLineStyle(linestyle[n]);
     gr->SetLineWidth(linewidth[n]);
     gr->SetLineColor(color[n]);
  */
  //Styling parameters for the "average line" and fill.
  Int_t avgLineWidth = 5;
  Int_t avgLineStyle = 10;
  Int_t avgLineColor = kBlack;
  Int_t avgFillStyle = 0;
  Int_t avgFillColor = icLGray;
  ///////////////////////////////////////////////////////////////////////
  //////////////////////// END NEW STYLING //////////////////////////////
  ///////////////////////////////////////////////////////////////////////

  
  //density of Fe at room temperature in g/cm^3
  const double RHO_Fe = 7.874;
  //magnetic saturation induction of Fe in kGaus at H_int=1.8 T
  const double H_sat = 218.0*(4*3.1415927*RHO_Fe)/1000.0;
  if(0){
    TGraph *grt = new TGraph();
    for(int i=1;i<100;++i)grt->SetPoint(i-1,i,magnetization(i,286.4));
    grt->Draw("acp");
    return;
  }

  TGaxis::SetMaxDigits(4);
  gStyle->SetPadRightMargin(0.075);
  gStyle->SetPadLeftMargin(0.11);

  TCanvas *c= new TCanvas("c","c",0,0,660,1200);
  TPad *pad1 = new TPad("pad1","",0,0.5,1,1);//c->Divide(1,2);
  TPad *pad2 = new TPad("pad2","",0,0,1,0.5);
  pad1->SetBottomMargin(0);
  pad2->SetTopMargin(0);
  pad1->Draw();
  pad2->Draw();

  pad1->cd();
  double upper_y = 219.1, lower_y = 213.4;
  double lower_limit = (use_Hi ? 0:2);
  double upper_limit = (use_Hi ? 28.9:0);
  int style[7] = {34,21,8,4,33,34,26};
  int color[9] = {kGray+2,kBlack,kGreen+3,kRed,kBlue,kViolet,kOrange+7,1};
  if(new_style)for(int i=0;i<9;++i)color[i] = color1[i];
  //int color[7] = {kBlue,kBlue+3,kBlue-4,kBlue-7,kAzure+7,kBlue-5,kViolet+9};
  const double pi = 3.1415927;

  //Weiss and Forrer 1929
  const int nWeis = 2;
  int n = 0;
  const double demagWeiss = 7.24;//taken to be that of a prolate ellipsoid of 
                                 // ratio 9:4 (see Weiss and Forrer pg 300)
  double xWeiss[nWeis] = {6000, 17000};//applied field H over range where a was
                                     //evaluated from 6kOe to 17 kOe.

  //Y-value uses eqs 1 and 30 from Weiss and Forrer with a=2.6 to evaluate at 
  //H=6000-17000 Oe. 
  double yWeiss[nWeis] = {217.76*(1-2.6/(xWeiss[0])),
			  217.76*(1-2.6/(xWeiss[1]))};

  if(use_Hi)
    for(int i=0;i<nWeis;++i)xWeiss[i] -= yWeiss[i]*4*pi*RHO_Fe/demagWeiss;
  for(int i=0;i<nWeis;++i)xWeiss[i] *= 0.001;

  TGraph *grWeiss = new TGraph(nWeis, xWeiss, yWeiss);
  grWeiss->SetMarkerColor(color[n]);
  grWeiss->SetMarkerSize(1.5);
  grWeiss->SetMarkerStyle(style[n]);
  if(new_style){
    grWeiss->SetMarkerSize(markersize[n]);
    grWeiss->SetMarkerStyle(markstyle[n]);
    grWeiss->SetLineStyle(linestyle[n]);  
    grWeiss->SetLineWidth(linewidth[n]);  
    grWeiss->SetLineColor(color[n]);
  }
  ++n;

  //Sanford 1941 single point taken as reported value at applied field H=inf
  //of 21.578 kOe minus a 9.86 Oe correction to get to H=10000 g. The correction
  //is estimated from Fig 6 of Sanford's 1941 paper "A Determination of the
  //Magnetic Saturation Induction of Iron at Room Temperature"
  //H=10000 g is the highest field of the range measured and it was chosen to
  //minimize the size of the correction from infinite H.
  
  const int nSan = 1;
  const double demagSanford = 32.1;//taken to be that of a cylinder of diameter
                                   //0.6 mm and length 8 mm (see Sanford pg 7 
                                   // and Table II and eq 2b  "Simple and 
                                   //approximate expressions of demagnetizing 
                                   //factors of uniformly magnetized 
                                   //rectangular rod and cylinder" M. Sato)

  double xSanford[nSan] = {10000};
  double ySanford[nSan] = {(21578-9.86*10000/xSanford[0])/(4*pi*RHO_Fe)};
  if(use_Hi)
    for(int i=0;i<nSan;++i)  
      xSanford[i] -= ySanford[i]*4*pi*RHO_Fe/demagSanford;
  for(int i=0;i<nSan;++i)xSanford[i] *= 0.001;

  TGraph *grSanford = new TGraph(nSan, xSanford, ySanford);
  grSanford->SetMarkerColor(color[n]);
  grSanford->SetMarkerSize(1.1);
  grSanford->SetMarkerStyle(style[n]);
  if(new_style){
    grSanford->SetMarkerColor(color[n]);    
    grSanford->SetMarkerSize(markersize[n]);
    grSanford->SetMarkerStyle(markstyle[n]);
    grSanford->SetLineStyle(linestyle[n]);  
    grSanford->SetLineWidth(linewidth[n]);  
    grSanford->SetLineColor(color[n]);      
  }
  ++n;

  //Danan 1959 (Demagnetizing factor same as Weiss and Forrer)
  const int nDan = 33, nDanC = 89;
  const double demagDanan = demagWeiss;
  double xDanan[nDan] = {0.03551, 0.04017, 0.09126, 0.04759, 0.05587, 0.06866, 0.08866, 0.12895, 0.16360, 0.19827, 0.21007, 0.20912, 0.25013, 0.22837, 0.29310, 0.36874, 0.3696480378516221, 0.4224972238925199, 0.5801450159543885, 0.6540048367912149, 0.7086403876455537, 0.7424452569161288, 0.7825550722665766, 0.8208960792145261, 0.8673240079530542, 0.9184241365537642, 1.0085609448860369, 1.0752138590307982, 1.1964847676189554, 1.2932205919143949, 1.4018157718017707, 1.5981908118523331,  1.7526586989821666};
  double yDanan[nDan] = {218.00568, 217.91716, 217.85066, 217.80878, 217.74197, 217.71134, 217.73859, 217.56358, 217.52407, 217.47372, 217.50634, 217.54248, 217.51386, 217.44141, 217.38405, 217.34483,217.34484082918930, 217.32353612451007, 217.25057936156114, 217.17701690243464, 217.18464077388660, 217.09633092956807, 217.06589798847423, 216.99207876683770, 217.0086794506599, 216.9277235479751,  216.95006188634858, 216.8113870881376, 216.69479386579002, 216.5328425584958,  216.35471192124194, 215.88986643960376, 215.43556138818542};//internal field H_i

  double xDananC[nDanC]={  0.02825, 0.04113, 0.05672, 0.0901963, 0.1102606, 0.1259643, 0.1448999, 0.1641936, 0.1875700, 0.2346587, 0.2545963, 0.2742439, 0.2919695, 0.3118779, 0.3325178, 0.3525502, 0.3725799, 0.3926154, 0.4126468, 0.4392051, 0.4599958, 0.4800294, 0.4999142, 0.5200964, 0.5401308, 0.5601636, 0.5801950, 0.6002320, 0.6202680, 0.6382160, 0.7004492, 0.6803710, 0.7243529, 0.7690656, 0.8078771, 0.8265169, 0.8443119, 0.8643436, 0.8821105, 0.9026114, 0.9178829, 0.9323183, 0.9536185, 0.9736579, 0.9944564, 1.0137445, 1.0337787, 1.0545762, 1.0920832, 1.1121254, 1.1321690, 1.1522111, 1.1722569, 1.1926044, 1.2123472, 1.2323942, 1.2524413, 1.2724889, 1.2925421, 1.3126005, 1.3326529, 1.3527078, 1.3727594, 1.3928138, 1.4128678, 1.4329222, 1.4529747, 1.4730294, 1.4930842, 1.5131387, 1.5331954, 1.5532513, 1.5733073, 1.5940976, 1.6134158, 1.6306757, 1.6517158, 1.6717763, 1.6918344, 1.7118955, 1.7319554, 1.7511069, 1.7702589, 1.7903196, 1.8103815, 1.8304434, 1.8495940, 1.8678364, 1.8869925};//internal field H_i
  double yDananC[nDanC] = {  217.9821, 217.8810, 217.7871, 217.6644, 217.6244, 217.5956, 217.5693, 217.5394, 217.5190, 217.4762, 217.4620, 217.4440, 217.4319, 217.4203, 217.4057, 217.3919, 217.3803, 217.3640, 217.3509, 217.3325, 217.3187, 217.3039, 217.2867, 217.2745, 217.2590, 217.2449, 217.2319, 217.2143, 217.1975, 217.1846, 217.1349, 217.1514, 217.1116, 217.0741, 217.0412, 217.0216, 217.0060, 216.9928, 216.9715, 216.9466, 216.9333, 216.9191, 216.8986, 216.8791, 216.8587, 216.8335, 216.8183, 216.7988, 216.7547, 216.7329, 216.7099, 216.6880, 216.6632, 216.6396, 216.6146, 216.5888, 216.5629, 216.5366, 216.5057, 216.4704, 216.4402, 216.4078, 216.3783, 216.3463, 216.3147, 216.2829, 216.2525, 216.2203, 216.1881, 216.1561, 216.1222, 216.0891, 216.0559, 216.0173, 215.9923, 215.9564, 215.9196, 215.8826, 215.8477, 215.8102, 215.7738, 215.7362, 215.6982, 215.6611, 215.6230, 215.5849, 215.5480, 215.5099, 215.4685};
  
  for(int i=0;i<nDan;++i)
    xDanan[i] = 1000/xDanan[i]+(use_Hi ? 0:4*pi*yDanan[i]*RHO_Fe/demagDanan);
  for(int i=0;i<nDanC;++i)
    xDananC[i] = 1000/xDananC[i]+(use_Hi ? 0:4*pi*yDananC[i]*RHO_Fe/demagDanan);

  for(int i=0;i<nDan;++i)xDanan[i] *= 0.001;
  for(int i=0;i<nDanC;++i)xDananC[i] *= 0.001;  
  TGraph *grDanan = new TGraph(nDan, xDanan, yDanan);
  grDanan->SetMarkerColor(color[n]);
  grDanan->SetMarkerStyle(style[n]);
  grDanan->SetLineColor(color[n]);       
  if(new_style){
    grDanan->SetMarkerColor(color[n]);     
    grDanan->SetMarkerStyle(markstyle[n]); 
    grDanan->SetMarkerSize(markersize[n]); 
    grDanan->SetLineStyle(linestyle[n]);   
    grDanan->SetLineWidth(linewidth[n]);   
  }

  TGraph *grDananC = new TGraph(nDanC, xDananC, yDananC);
  grDananC->SetLineColor(color[n]);
  grDananC->SetMarkerColor(color[n]);
  grDananC->SetMarkerStyle(style[n]);
  if(new_style){
    grDananC->SetMarkerColor(color[n]);     
    grDananC->SetMarkerStyle(markstyle[n]); 
    grDananC->SetMarkerSize(markersize[n]); 
    grDananC->SetLineStyle(linestyle[n]);   
    grDananC->SetLineWidth(linewidth[n]);   
  }
  grDanan->GetXaxis()->SetLimits(lower_limit,upper_limit);
  grDanan->GetYaxis()->SetLimits(lower_y,upper_y);
  grDanan->GetYaxis()->SetTitleOffset(1.5);
  grDanan->GetYaxis()->SetRangeUser(lower_y,upper_y);
  grDanan->Draw("ap");
  grDanan->SetTitle(Form("Magnetization of Iron vs %s",(use_Hi ? "Internal Field H_{int}":"Applied Field H")));
  grDanan->GetYaxis()->SetTitle("Magnetization (emu/g)");
  grDanan->GetYaxis()->SetTitleOffset(0.92);
  grDanan->GetYaxis()->SetTitleSize(0.042);
  grDanan->GetXaxis()->SetTitle((use_Hi ? "H_{int} (kOe)" : "H (kOe)"));
  //grDananC->Draw("samec");
  grSanford->Draw("samep");
  ++n;
  const int nAra = 15;
  const double demagAraj = 3.0;
  //Taken from Fig 2 on "A Note on the Consistency of Values of the Spontaneous or Saturation Magnetization of Polycrystalline Iron and Nickel at 298 K
  double xAraj[nAra] = {2.531894698,2.916384427,3.307673144,3.651750519,4.196398050,4.891906807,6.399513097,7.378185843,8.788178086,9.667641104,11.16196162,13.53678581,15.81198401,17.95432442,20.29563931};
  double yAraj[nAra] = {462.5038946,465.9467307,467.7528650,468.9240569,469.8125422,470.3846159,471.4626655,471.8694443,472.4458572,472.7351233,473.0114220,473.3264713,473.6242140,473.9044484,474.2860999};
  for(int i=0;i<nAra;++i){
    xAraj[i]*=217.10;
    yAraj[i]=sqrt(100*yAraj[i]);
  }
  //Taken from Fig 3 on "A Note on the Consistency of Values of the Spontaneous or Saturation Magnetization of Polycrystalline Iron and Nickel at 298 K
  //double xAraj[nAra] = {1.83963, 1.59581, 1.38579, 1.24863, 1.10280, 0.92821, 0.71221, 0.61922, 0.51771, 0.47053, 0.41331, 0.33462, 0.28456, 0.25172, 0.23176};//1/H_i
  //double yAraj[nAra] = {215.06297, 215.87245, 216.27994, 216.54179, 216.75649, 216.88718, 217.12381, 217.21786, 217.36050, 217.42667, 217.48839, 217.56186, 217.62213, 217.69717, 217.77076};

  //  for(int i=0;i<nAra;++i)
  //  xAraj[i] = 1000/xAraj[i] + (use_Hi ? 0:4*pi*yAraj[i]*RHO_Fe/demagAraj);
  for(int i=0;i<nAra;++i)xAraj[i] *= 0.001;
  TGraph *grAraj = new TGraph(nAra, xAraj, yAraj);
  grAraj->SetMarkerColor(color[n]);
  grAraj->SetMarkerStyle(style[n]);
  grAraj->SetLineColor(color[n]);
  if(new_style){
    grAraj->SetMarkerStyle(markstyle[n]); 
    grAraj->SetMarkerSize(markersize[n]); 
    grAraj->SetLineStyle(linestyle[n]);   
    grAraj->SetLineWidth(linewidth[n]);   
  }
  grAraj->Draw("samep");
  ++n;

  const int nCra = 8;
  const double demagCrangle = 5.76;
  double xCrangle[nCra] = {4000,5000,6000,7000,8000,9000,10000,10500};//applied field H
  double yCrangle[nCra] = {213.5, 217.5, 217.65, 217.7, 217.55, 217.75, 217.6, 217.7};
  if(use_Hi)
    for(int i=0;i<nCra;++i)
      xCrangle[i] -= yCrangle[i]*4*pi*RHO_Fe/demagCrangle;
  for(int i=0;i<nCra;++i)xCrangle[i] *= 0.001;
  TGraph *grCrangle = new TGraph(nCra, xCrangle, yCrangle);
  grCrangle->SetMarkerColor(color[n]);
  grCrangle->SetMarkerStyle(style[n]);
  grCrangle->SetMarkerSize(1.5);
  grCrangle->SetLineColor(color[n]);
  if(new_style){
    grCrangle->SetMarkerStyle(markstyle[n]); 
    grCrangle->SetMarkerSize(markersize[n]); 
    grCrangle->SetLineStyle(linestyle[n]);   
    grCrangle->SetLineWidth(linewidth[n]);   
  }
  grCrangle->Draw("samep");
  grWeiss->Draw("samep");
  ++n;

  const int nNasa = 26;
  const double demagNasa = 3.0;
  double xNasa[nNasa] = {0.18380,0.28653,0.37976,0.50229,0.60599,0.69708,0.80355,0.90037,0.99829,1.10086,1.19899,1.29509,1.39033,1.49674,1.60303,1.69727,1.79041,1.89866,2.00191,2.09468,2.19793,2.29533,2.40148,2.50363,2.59619,2.70248};
  double yNasa[nNasa] = {217.38795, 217.61164, 217.70140, 217.73676, 217.78903, 217.80291, 217.86079, 217.90370, 217.92225, 217.99606, 218.03522, 218.00788, 218.02925, 218.34755, 218.38763, 218.31064, 218.25894, 218.49106, 218.49929, 218.41200, 218.42024, 218.52029, 218.67934, 218.44683, 218.60404,218.64412};

  for(int i=0;i<nNasa;++i)
    xNasa[i] = 10000*xNasa[i] + (use_Hi ? 0:4*pi*yNasa[i]*RHO_Fe/demagNasa);
  for(int i=0;i<nNasa;++i)xNasa[i] *= 0.001;
  TGraph *grNasa = new TGraph(nNasa, xNasa, yNasa);
  grNasa->SetMarkerColor(color[n]);
  grNasa->SetMarkerStyle(style[n]);
  grNasa->SetLineColor(color[n]);
  if(new_style){
    grNasa->SetMarkerSize(markersize[n]);
    grNasa->SetMarkerStyle(markstyle[n]);
    grNasa->SetLineStyle(linestyle[n]);
    grNasa->SetLineWidth(linewidth[n]);
  }
  grNasa->Draw("samep");
  ++n;
  TLegend *leg = new TLegend(0.56, 0.13, 0.9, 0.6);
  leg->SetFillColor(0);
  leg->SetBorderSize(0);
  leg->SetShadowColor(0);
  leg->AddEntry(grWeiss, "Weiss #it{et al.} (1929)","pl");
  leg->AddEntry(grSanford, "NIST (1941)","pl");
  leg->AddEntry(grDanan, "Henri Danan (1959)","pl");
  leg->AddEntry(grAraj, "Arajs #it{et al.} (1967)","pl");
  leg->AddEntry(grCrangle, "Crangle #it{et al.} (1970)","pl");
  leg->AddEntry(grNasa, "NASA (1972)","plf");
  leg->Draw();
  double x_ = 5, y_ = 216.5, x_err = 0, y_err = y_*0.002;
  TGraphErrors *gr02 = new TGraphErrors(1,&x_,&y_,&x_err,&y_err);
  gr02->SetLineWidth(3);
  gr02->Draw("samep");
  TPaveText *pte = new TPaveText(0.21,0.46,0.3,0.52,"ndc");
  pte->SetBorderSize(0);
  pte->SetShadowColor(0);
  pte->SetFillColor(0);
  pte->AddText("0.4%");
  pte->Draw();
  TPaveText *tp1 = new TPaveText(0.2,0.1,0.5,0.25,"ndc");
  tp1->SetShadowColor(0);
  tp1->SetFillColor(0);
  tp1->AddText("Temperature Range");
  tp1->AddText("288_{ }K to 299_{ }K");
  tp1->Draw();
  pad2->cd()->SetTopMargin(0);

  n=0;
  //Now correct to T=294 K
  double T = 294;
  for(int i=0;i<nWeis;++i){//from 288
    double corrWeiss = magnetization(xWeiss[i], T);
    corrWeiss -= magnetization(xWeiss[i], 288);
    yWeiss[i] += corrWeiss;
  }
  for(int i=0;i<nSan;++i){//from 298
    double corrSanford = magnetization(xSanford[i], T);
    corrSanford -= magnetization(xSanford[i], 298);
    ySanford[i] += corrSanford;
  }
  for(int i=0;i<nDan;++i){//from 288
    double corrDanan = magnetization(xDanan[i], T);
    corrDanan -= magnetization(xDanan[i], 288);
    yDanan[i] += corrDanan;
  }
  for(int i=0;i<nDanC;++i){//from 288
    double corrDanan = magnetization(xDananC[i], T);
    corrDanan -= magnetization(xDananC[i], 288);
    yDananC[i] += corrDanan;
  }
  for(int i=0;i<nAra;++i){//from 298
    double corrAraj = magnetization(xAraj[i], T);
    corrAraj -= magnetization(xAraj[i], 298);
    yAraj[i] += corrAraj;
  }
  for(int i=0;i<nCra;++i){//from 293
    double corrCrangle = magnetization(xCrangle[i], T);
    corrCrangle -= magnetization(xCrangle[i], 293);
    yCrangle[i]+=corrCrangle;
  }
  for(int i=0;i<nNasa;++i){//from 299
    double corrNasa = magnetization(xNasa[i], T);
    corrNasa -= magnetization(xNasa[i], 299);
    yNasa[i]+=corrNasa;
  }
  TGraph *grWeiss2 = new TGraph(nWeis, xWeiss, yWeiss);
  grWeiss2->SetMarkerColor(color[n]);
  grWeiss2->SetMarkerSize(1.5);
  grWeiss2->SetMarkerStyle(style[n]);
  if(new_style){
    grWeiss2->SetMarkerSize(markersize[n]);
    grWeiss2->SetMarkerStyle(markstyle[n]);
    grWeiss2->SetLineStyle(linestyle[n]); 
    grWeiss2->SetLineWidth(linewidth[n]); 
    grWeiss2->SetLineColor(color[n]);     
  }
  ++n;
  TGraph *grSanford2 = new TGraph(nSan, xSanford, ySanford);
  grSanford2->SetMarkerColor(color[n]);
  grSanford2->SetMarkerSize(1);
  grSanford2->SetMarkerStyle(style[n]);
  if(new_style){
    grSanford2->SetMarkerSize(markersize[n]);
    grSanford2->SetMarkerStyle(markstyle[n]);
    grSanford2->SetLineStyle(linestyle[n]); 
    grSanford2->SetLineWidth(linewidth[n]); 
    grSanford2->SetLineColor(color[n]);     
  }
  ++n;

  TGraph *grDanan2 = new TGraph(nDan, xDanan, yDanan);
  grDanan2->SetMarkerColor(color[n]);
  grDanan2->SetMarkerStyle(style[n]);
  if(new_style){
    grDanan2->SetMarkerStyle(markstyle[n]);
    grDanan2->SetMarkerSize(markersize[n]); 
    grDanan2->SetLineWidth(linewidth[n]);   
    grDanan2->SetLineStyle(linestyle[n]);   
    grDanan2->SetLineColor(color[n]);       
  }
  TGraph *grDananC2 = new TGraph(nDanC, xDananC, yDananC);
  grDananC2->SetLineColor(color[n]);
  grDananC2->SetMarkerColor(color[n]);
  grDananC2->SetMarkerStyle(style[n]);
  if(new_style){
    grDananC2->SetMarkerStyle(markstyle[n]);
    grDananC2->SetLineStyle(linestyle[n]);  
    grDananC2->SetLineWidth(linewidth[n]);  
    grDananC2->SetMarkerSize(markersize[n]);
    grDananC2->SetLineColor(color[n]);       
  }
  grDanan2->GetXaxis()->SetLimits(lower_limit,upper_limit);
  grDanan2->GetYaxis()->SetLimits(lower_y,upper_y);
  grDanan2->GetYaxis()->SetTitleOffset(1.5);
  grDanan2->GetYaxis()->SetRangeUser(lower_y,upper_y);
  grDanan2->SetTitle("");
  grDanan2->Draw("ap");
  //grDanan2->SetTitle(Form("Magnetization of Iron at 294 K versus %s",(use_Hi ? "H_{int}":"H")));
  grDanan2->GetXaxis()->SetTitleSize(0.042);
  grDanan2->GetYaxis()->SetTitleSize(0.042);
  grDanan2->GetYaxis()->SetTitleOffset(0.92);
  grDanan2->GetYaxis()->SetTitle("Magnetization (emu/g)");
  grDanan2->GetXaxis()->SetTitle((use_Hi ? "H_{int} (kOe)":"H (kOe)"));
  //  grDananC2->Draw("samec");
  grSanford2->Draw("samep");
  ++n;
  TGraph *grAraj2 = new TGraph(nAra, xAraj, yAraj);
  grAraj2->SetMarkerColor(color[n]);
  grAraj2->SetMarkerStyle(style[n]);
  grAraj2->SetLineColor(color[n]);
  if(new_style){
    grAraj2->SetMarkerStyle(markstyle[n]);
    grAraj2->SetMarkerSize(markersize[n]); 
    grAraj2->SetLineStyle(linestyle[n]);   
    grAraj2->SetLineWidth(linewidth[n]);   
    grAraj2->SetLineColor(color[n]);       
  }
  grAraj2->Draw("samep");
  ++n;
  TGraph *grCrangle2 = new TGraph(nCra, xCrangle, yCrangle);
  grCrangle2->SetMarkerColor(color[n]);
  grCrangle2->SetMarkerSize(1.5);
  grCrangle2->SetMarkerStyle(style[n]);
  grCrangle2->SetLineColor(color[n]);
  if(new_style){
    grCrangle2->SetMarkerSize(markersize[n]);
    grCrangle2->SetMarkerStyle(markstyle[n]);
    grCrangle2->SetLineStyle(linestyle[n]); 
    grCrangle2->SetLineWidth(linewidth[n]); 
    grCrangle2->SetLineColor(color[n]);     
  }
  grCrangle2->Draw("samep");
  ++n;
  TGraph *grNasa2 = new TGraph(nNasa, xNasa, yNasa);
  grNasa2->SetMarkerColor(color[n]);
  grNasa2->SetMarkerStyle(style[n]);
  grNasa2->SetLineColor(color[n]);
  if(new_style){
    grNasa2->SetMarkerStyle(markstyle[n]);
    grNasa2->SetLineStyle(linestyle[n]);    
    grNasa2->SetLineWidth(linewidth[n]);    
    grNasa2->SetLineColor(color[n]);        
    grNasa2->SetMarkerSize(markersize[n]);  
  }

  grNasa2->Draw("samep");
  grWeiss2->Draw("samep");
  leg->Draw();
  gr02->Draw("samep");
  TPaveText *pte2 = new TPaveText(0.21,0.56,0.3,0.62,"ndc");
  pte2->SetBorderSize(0);
  pte2->SetShadowColor(0);
  pte2->SetFillColor(0);
  pte2->AddText("0.4%");
  pte2->Draw();
  TPaveText *tp2 = new TPaveText(0.2,0.25,0.5,0.35,"ndc");
  tp2->SetShadowColor(0);
  tp2->SetFillColor(0);
  tp2->AddText("Corrected to 294_{ }K");
  tp2->Draw();

  // TF1 *f = new TF1("f","[0]+[1]*sqrt(x)+[2]*x",100,20000);
  // f->SetParameters(218, 1, 0);
  // grDanan->Fit(f,"r");
  c->SaveAs(Form("FeMagnetization_vs_%s.pdf",(use_Hi ? "Hint":"H")));
  c->SaveAs(Form("../nim/figures/FeMagnetization_vs_%s.pdf",(use_Hi ? "Hint":"H")));

  TCanvas *cAll = new TCanvas("cAll","cAll",0,0,700,500);
  TGraph *grAll = new TGraph();
  n=0;
  for(int i=0;i<nWeis;++i)grAll->SetPoint(i, xWeiss[i], yWeiss[i]);
  n+=nWeis;
  for(int i=0;i<nSan;++i){
    cout<<xSanford[i]<<" "<<ySanford[i]<<endl;
    grAll->SetPoint(i+n, xSanford[i], ySanford[i]);
  }
  n+=nSan;
  for(int i=0;i<nDan;++i)grAll->SetPoint(i+n, xDanan[i], yDanan[i]);
  n+=nDan;
  for(int i=0;i<nAra;++i)grAll->SetPoint(i+n, xAraj[i], yAraj[i]);
  n+=nAra;
  for(int i=0;i<nCra;++i)grAll->SetPoint(i+n, xCrangle[i], yCrangle[i]);
  n+=nCra;
  for(int i=0;i<nNasa;++i)grAll->SetPoint(i+n, xNasa[i], yNasa[i]);
  n+=nNasa;

  grAll->SetMarkerColor(kGray+2);
  grAll->SetMarkerStyle(8);
  grAll->Draw("ap");
  grAll->SetTitle(Form("Magnetization of Iron at 294 K vs %s",(use_Hi ? "H_{int}":"H")));
  grAll->GetYaxis()->SetTitle("Magnetization (emu/g)");
  grAll->GetXaxis()->SetTitle((use_Hi ? "H_{int} (kOe)":"H (kOe)"));
  grAll->GetYaxis()->SetTitleOffset(1.4);
  grAll->GetXaxis()->SetTitleSize(0.042);
  grAll->GetYaxis()->SetTitleSize(0.042);
  grAll->GetXaxis()->SetRangeUser(lower_limit,upper_limit);
  grAll->GetYaxis()->SetRangeUser(lower_y,upper_y);
  gPad->Update();
  TF1 *f = new TF1("f","magnetization(x+[1],294,[0])",0,29);
  f->SetLineColor(kGray);
  f->SetParameters(218,-1.9);
  grAll->Fit(f,"r");
  gStyle->SetOptFit(0);
  printf("Results from fitting all points at once.\n");
  printf("Fit Msat: %f, offset: %f\n",f->GetParameter(0),f->GetParameter(1));
  printf("Evaluated at 18kOe: %f\n",f->Eval(18));
  
  TCanvas *cnim = new TCanvas("cnim","cnim", 0,0,800,600);  
  if(1){
    TCanvas *cx = new TCanvas("cx","cx",0,0,660,1200);
    TPad *pad1x = new TPad("pad1x","",0,0.5,1,1);
    TPad *pad2x = new TPad("pad2x","",0,0,1,0.5);
    pad1x->SetBottomMargin(0);
    pad2x->SetTopMargin(0);
    pad1x->Draw();
    pad2x->Draw();

    pad1x->cd();
    TF1 *fx = new TF1("fx","magnetization(x,294,[0])+[1]/x/x",0,29);
 
    TGraph *grf[6];
    TMultiGraph *mg = new TMultiGraph();

    grf[0] = (TGraph*)grWeiss2->Clone();
    grf[1] = (TGraph*)grSanford2->Clone();
    grf[2] = (TGraph*)grDanan2->Clone();
    grf[3] = (TGraph*)grAraj2->Clone();
    grf[4] = (TGraph*)grCrangle2->Clone();
    grf[5] = (TGraph*)grNasa2->Clone();
    double par[2] = {0,0}, offset=1.9;
    for(int i=5;i>=0;--i){
      if(i<2){
	//fx->SetParameters(218,-offset);
	//fx->FixParameter(1,-offset);
	fx->SetParameters(par[0],par[1]);
	cout<<"Parameter fixed to "<<par[1]<<endl;
	fx->FixParameter(1, par[1]);
      }else{
	//fx->ReleaseParameter(1);
	//fx->SetParameters(218,-offset);
	fx->ReleaseParameter(1);
	fx->SetParameters(218,-0.7);
	fx->SetParLimits(1,-10,0);
      }
      fx->SetLineColor(color[i]);
      fx->SetLineStyle(linestyle[i]);
      fx->SetRange(lower_limit,upper_limit);
      grf[i]->Fit(fx,"rB");
      par[0]+=fx->GetParameter(0)/6.0;
      if(i>1){
	par[1]+=fx->GetParameter(1)/4.0;
      }
      mg->Add(grf[i]); 
    }
    mg->Draw("ap");
    mg->SetTitle(Form("Magnetization of Iron at 294 K vs %s",(use_Hi ? "H_{int}":"H")));
    mg->GetYaxis()->SetTitle("Magnetization (emu/g)");
    mg->GetXaxis()->SetTitle((use_Hi ? "H_{int} (kOe)":"H (kOe)"));
    mg->GetYaxis()->SetTitleSize(0.042);
    mg->GetXaxis()->SetLimits(lower_limit,upper_limit);
    mg->GetXaxis()->SetRangeUser(lower_limit,upper_limit);
    mg->GetYaxis()->SetRangeUser(lower_y, upper_y);
    fx->SetLineColor(kBlack);
    fx->SetLineWidth(4);
    fx->SetLineStyle(10);
    if(new_style){
      fx->SetLineColor( avgLineColor );
      fx->SetLineWidth( avgLineWidth );
      fx->SetLineStyle( avgLineStyle );
    }
    fx->SetParameters(par[0],par[1]);
    fx->Draw("same");
    leg->AddEntry(fx, "Average","l");
    leg->Draw();
    gr02->Draw("samep");
    pte->Draw();
    pad2x->cd();

    double x[100],xe[100],y[100],ye[100];
    for(int i=0;i<100;++i){
      x[i] = (i+2)*0.284;
      xe[i] = 0;
      y[i] = fx->Eval(x[i]);
      ye[i] = 0.002*y[i];
    }
    TGraph *grAll2 = new TGraph();
    grAll2->SetMarkerStyle(8);
    double xt, yt;
    for(int i=0;i<grAll->GetN();++i){
      grAll->GetPoint(i, xt, yt);
      grAll2->SetPoint(i, xt, yt);
    }
    TGraphErrors *gr = new TGraphErrors(100,x,y,xe,ye);
    TGraph *gr1 = new TGraph(100,x,y);
    gr1->SetLineColor(kBlue);
    gr1->SetLineWidth(2);
    gr->SetFillColor(icSoft);
    gr->SetTitle(Form("Magnetization of Iron at 294 K vs %s",(use_Hi ? "H_{int}":"H")));
    //gr->SetTitle("");
    gr->Draw("3a");
    gr->GetYaxis()->SetTitleSize(0.042);
    gr->GetXaxis()->SetTitleSize(0.042);
    gPad->Update();
    gr->GetYaxis()->SetTitle("Magnetization (emu/g)");
    gr->GetXaxis()->SetTitle((use_Hi ? "H_{int} (kOe)":"H (kOe)"));
    gr->GetXaxis()->SetLimits(lower_limit,upper_limit);
    gr->GetXaxis()->SetRangeUser(lower_limit,upper_limit);
    gr->GetYaxis()->SetRangeUser(lower_y, upper_y);
    gr02->Draw("samep");
    pte2->Draw();
    gr1->Draw("samel");
    grAll2->Draw("samep");
    TArrow *ar4 = new TArrow(8,213.75,28,213.75,0.025,"<|>");
    ar4->SetAngle(40);
    ar4->SetLineWidth(2);
    ar4->SetFillColor(1);
    ar4->Draw();
    gPad->Update();
    TPaveText *tp3 = new TPaveText(0.46,0.12,0.77,0.18,"ndc");
    tp3->SetBorderSize(0);
    tp3->SetShadowColor(0);
    tp3->SetFillColor(0);
    tp3->AddText("Region of interest");
    tp3->Draw();
    
    cnim->cd();
    gr->SetFillColor(18);
    gr->SetFillColor( avgFillColor);
    gr->Draw("3a");
    for(int i=0;i<6;++i) grf[i]->Draw("samep");
    fx->Draw("same");
    ar4->Draw();
    gr02->Draw("samep");
    TPaveText *pte5 = new TPaveText(0.21,0.51,0.3,0.56,"ndc");
    pte5->SetBorderSize(0);
    pte5->SetShadowColor(0);
    pte5->SetFillColor(0);
    pte5->AddText("0.4%");
    pte5->Draw();
    gPad->Update();
    leg->SetY1NDC(0.24);
    leg->Draw();
    tp3->Draw();
    cnim->ForceUpdate();
    if(use_Hi){
      cx->Print("FeCombinedFitErrorBand_vs_Hint.pdf");
      cx->Print("../nim/figures/FeCombinedFitErrorBand_vs_Hint.pdf");
      cnim->Print("../nim/figures/FeComboPlotWithErrorBand.pdf");
    }else{
      cx->Print("FeCombinedFitErrorBand_vs_H.pdf");
      cx->Print("../nim/figures/FeCombinedFitErrorBand_vs_H.pdf");
    }
    TCanvas *cxx = new TCanvas("cxx","cxx",0,0,700,500);
    TGraph *grxx = new TGraph();
    for(int i=0;i<100;++i)
      grxx->SetPoint(i,8+i*0.2,fx->Eval(8+i*0.2));
    grxx->SetMarkerStyle(8);
    grxx->Draw("ap");
    TF1 *p2 = new TF1("p2","pol2",0,1);
    grxx->Fit("p2");
    TPaveText *pt = new TPaveText(0.5,0.2,0.89,0.4,"ndc");
    pt->SetShadowColor(0);
    pt->SetBorderSize(0);
    pt->SetFillColor(0);
    double p_0 = p2->GetParameter(0);
    double p_1 = p2->GetParameter(1);
    double p_2 = p2->GetParameter(2);
    pt->AddText("With H_{int} in kOe:");
    pt->AddText(Form("%0.3f%+0.4eH_{int}%+0.4eH_{int}^{2}", p_0, p_1, p_2));
    pt->AddText("With H in tesla:");
    pt->AddText(Form("%0.3f%+0.6fH%+0.6fH^{2}",
		     p_0 - p_1*H_sat + p_2*H_sat*H_sat,
		     (p_1 - 2*p_2*H_sat)*10, p_2*100));
    printf("%0.3f%+0.4eH_{int}%+0.4eH_{int}^{2}\n", p_0, p_1, p_2);
    printf("%0.3f%+0.6fH%+0.6fH^{2}\n", p_0 - p_1*H_sat + p_2*H_sat*H_sat,
	   (p_1 - 2*p_2*H_sat)*10, p_2*100);
    printf("Pol2 approx H=4T (Hi=%fkGaus) on thin foil: %f (emu/g)\n",
	   40-H_sat, p2->Eval(40-H_sat));
    pt->Draw();
    printf("Actual curve H=4T: Msat(H_i=%fgaus), (with %0.4e/H^2) = %f\n",
	   40-H_sat, fx->GetParameter(1), fx->Eval(40-H_sat));
    gPad->Update();
    cxx->SaveAs(Form("%i.pdf",int(offset)));

    TCanvas *c2 = new TCanvas("c2","c2",0,600,1000,660);
    c2->SetGrid();
    TF1 *f = new TF1("f","magnetization(x,294,[0])+[1]/x/x",0.2,20);
    TF1 *f1 = new TF1("f1","magnetization(x+[1],294,[0])",0.2,29);
    TF1 *f2 = new TF1("f2","magnetization(x+[1],294,[0])",0.5,29);
    TF1 *f3 = new TF1("f3","magnetization(x+[1],294,[0])",0.2,15);
    f->SetParameters(222,0.5);
    f1->SetParameters(222, 2.130);
    f2->SetParameters(222, 1.931);
    f3->SetParameters(222, 1.925);
    f->SetLineColor(kBlue);
    f1->SetLineColor(kBlack);
    f2->SetLineColor(kRed);
    f3->SetLineColor(kGreen+2);
    grAll->Draw("ap");
    grAll->Fit(f,"r");
    grAll->Fit(f1,"r");
    grAll->Fit(f2,"r");
    grAll->Fit(f3,"r");
    f1->Draw("same");
    f2->Draw("same");
    f3->Draw("same");
    f->Draw("same");
    c2->ForceUpdate();
    if(use_Hi)
      c2->Print("FeCombinedFit_vs_Hint.pdf");
    else
      c2->Print("FeCombinedFit_vs_H.pdf");
  }
  
  
  return;
}
