...
double NiFoilHeating(double beam_cur = 1e-6, double beam_r=10e-3, double beam_E = 11, double T0 = 294, double foil_r = 0.635, bool uniform = 0){
  gStyle->SetStatY(0.7);
  gStyle->SetStatH(0.2);
  ...
  //Use ESTAR data to estimate collision energy loss as a function of electron energy
  //----------------------------------------------------------------------------------
  ...
  double stop_en[12]={1.862,1.914,1.965,1.995,2.016, //collision stopping power 
		      2.032,2.045,2.057,2.066,2.075,//in (MeV cm^2/g) using ESTAR
		      2.083,2.091};
  ...
  cout<<"Total correction to magnetization for Ni: "<<-0.0238*(avg-T0)<<" emu/g"<<endl;
  return avg;
}
