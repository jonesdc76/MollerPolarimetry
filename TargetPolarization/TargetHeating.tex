...
\end{cases}
\end{equation}
Integrating both sides with respect to $r$ in reverse direction from $r=R_{foil}$ to $r\leq r_{rast}$ gives $\Delta T$ which can be solved analytically:
\begin{align}
\Delta T&=-\frac{\gamma r_{rast}^2}{2}\int_{R_{foil}}^{r_{rast}}\frac{dr}{r}-\frac{\gamma}{2}\int_{r_{rast}}^r r^\prime dr^\prime\\
\label{eq:uniform_dT}
~&= \frac{\gamma r_{rast}^2}{2}\ln{\left(\frac{R_{foil}}{r_{rast}}\right)}+\frac{\gamma}{4}\left(r_{rast}^2-r^2\right).
\end{align}
Figures \ref{fig:foilheatingT_uni} and \ref{fig:foilheatingT_uni} give plots of $\Delta T$ and $T$ respectively for a uniformly rastered beam for the parameters given on the plots.
