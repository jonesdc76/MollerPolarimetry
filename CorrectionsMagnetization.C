...
Arguments: meas_T is the temperature in Kelvin at which the magnetization 
	   ...
	   was measured (If the applied field is assumed to be 10000 Oersteds =
	   1 Telsa, then the internal field is 17177 Oe. Above saturation, the
           relation is a constant offset of 7.177 Oe. Below saturation the
           relationship is not so simple and this program is insufficient.)
...
-------------------------------------------------------------------------------
      ...
      in terms of applied field we can use H = H_i+M/3 to translate his 
      ...
      Crangle and Goodman(1970) so 4piM/3= 7.177 kOe. Thus, to translate 
      from internal field to H you add 7.177 kOe. My default field is H=10 kOe 
      which gives H_i=17.72 kOe and my default temperature is 293 K.
*/ 
#include <iostream>
#include <cstdio>
...
double CorrectionsMagnetization(double dT, double dH, //in Kelvin and Oersted
				double meas_T = 293, double meas_Hi = 17177){
