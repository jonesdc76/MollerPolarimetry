...
%% bibitems, please use
\appendix
\section{\label{app:target_heating}Calculations for Target Heating}
This analysis picks up from Eq. \ref{eq:heat_T_r} and provides detailed solutions for two electron beam distributions centered on a circular target foil: a beam with a circular Gaussian charge distribution and  a uniform charge distribution from a circularly rastered beam. These solutions could be extended to more difficult geometries. Although analytical solutions may not exist for more complex beam distributions or foil geometries, numerical solutions should not be difficult to calculate following similar steps to those given here.

\subsection{Gaussian profile beam spot}
Here we assume a Gaussian beam flux profile of radius $r_b$. Therefore, the Gaussian profiled electron flux $B_{\rm flux}$ from a beam current $I$ in amperes with a $1~\sigma$ radius of $r_{b}$ becomes
\begin{equation}
...
\]
where the temperature dependent $\kappa$ for Fe has been used. Now to find the temperature difference between the outside perimeter of the foil at $r=R_{\rm foil}$ and some $r<R_{\rm foil}$ integrate both sides from $R_{\rm foil}$ to $r$ yielding
\begin{equation}
\label{eq:dT}
\Delta T = \int_{R_{\rm foil}}^r \left(\frac{r_b^2\gamma}{r^{\prime}} e^{-r^{\prime 2}/2r_b^2}+\frac{C}{r^{\prime}}\right)dr^{\prime}.
\end{equation}
This can easily be integrated numerically as shown in Fig.  \ref{fig:foilheatingT}.
\begin{figure}[h]
%Figure made by FeFoilHeating.C
\centering
\includegraphics[width=0.7\linewidth]{FeFoilHeatingT.pdf}
...
\end{figure}
\subsection{Uniform circular beam distribution}
In the case where a beam is rastered, the charge distribution can be considered to be uniform. Solving for the case of a uniform circular raster pattern of radius $r_{\rm rast}$ centered on the foil yields an electron flux density $B$ given by
\begin{equation}
\label{eq:uniform}
...
\end{cases}.
\end{equation}
Fig. \ref{fig:foilheatingT_uni} shows $T$ for a uniformly rastered beam for the parameters given on the plot.
\begin{figure}[h]
%Figure made by FeFoilHeating.C
\centering
\includegraphics[width=0.7\linewidth]{FeFoilHeatingTUniform.pdf}
